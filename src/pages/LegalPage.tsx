  ...
  const getLegalContent = () => {
    if (type === 'privacy') {
      return {
        ...
        body: 'מערכת MyluimSync מחויבת להגנה מקסימלית על פרטיות הסטודנטים המשרתים במילואים והתומכים האקדמיים כאחד. כל המידע המועלה — לרבות שמות מלאים, כתובות דואר אלקטרוני, שיוך פקולטטי, התמחות, שנת לימוד, הגדרות תקופות שירות מילואים, נתוני התקדמות השלמת חומרים אקדמיים, הגדרות התראות וקבצי לימוד — נשמר ומאובטח במסדי הנתונים בענן של Supabase תחת הצפנה מתקדמת במצב מנוחה ובמעבר. המידע משמש אך ורק לצרכים פדגוגיים אקדמיים של המוסד ולטובת ליווי וסיוע לסטודנטים המשרתים במילואים, ואינו מועבר, נמכר או נחשף לשום גורם מסחרי או צד שלישי אחר.'
      }
    }
    return {
      title: 'תנאי שימוש ותקנון המערכת - MyluimSync',
      body: 'השימוש בפלטפורמת MyluimSync מיועד לסטודנטים פעילים בשירות מילואים ולמתנדבי מערך התמיכה בלבד. כל התכנים, הסיכומים, המטלות והפתרונות המועלים למערכת הם באחריותם הבלעדית של המשתמשים המעלים. המערכת עושה שימוש בנתוני תאריכי שיעורים והיעדרויות כדי לספק פילוחים ומדדי השלמה מותאמים אישית (כולל אחוזי השלמה ומספר החומרים שנותרו), וכן שולחת תזכורות פופאפ במקרה של חוסר פעילות ממושך (אשר ניתנות לכיבוי דרך עמוד הגדרות הפרופיל). חל איסור מוחלט על העלאת חומרים המפרים זכויות יוצרים מסחריות, ספרי לימוד מוגנים או תכנים מסווגים. המערכת שומרת לעצמה את הזכות להסיר חומרים שאינם עומדים בסטנדרט האקדמי של המוסד.'
    }
  }
